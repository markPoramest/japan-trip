...
import { X, Hotel, Calendar, FileText, ArrowRightLeft } from "lucide-react";
...
export default function HotelModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !dateRange.trim()) return;
    setLoading(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
      <div className="bg-bg-card border border-border rounded-3xl w-full max-w-md overflow-hidden shadow-2xl my-8 animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className={labelClass}>
              <Calendar className="w-3.5 h-3.5 text-accent" /> {t("checkInCheckOut")} *
            </label>
            <input
              required
              type="text"
              ...
              placeholder="e.g. 21-23 Oct (2 nights)"
              className={inputClass}
            />
          </div>
          ...
          <div className="pt-3 border-t border-border flex justify-end space-x-3">
            ...
            >
              {loading ? t("creating") : isEditing ? t("saveChanges") : t("addHotel")}
            </button>
          </div>
        </form>
      </div>
